...
export const listEventByName = async (
  calendarId: string,
  eventNamePattern?: RegExp | undefined,
  eventDate?: string | undefined,
  duration: number = 1
) => {

  let response = await listEvents(calendarId, eventDate, duration);
  const events = response.filter(item => eventNamePattern.test(item.summary));

  return events;
};

export const listEventById = async (
  calendarId: string,
  eventId: string,
  eventDate?: string | undefined,
  duration: number = 1
  ...
  let response = await listEvents(calendarId, eventDate, duration);
  const events = response.filter(item => item.id === eventId);

  return events;
};

export const appendEventInfo = async (
  calendarId: string,
  eventId: string,
  eventDate?: string | undefined,
  ...
  const enrolledPattern = /Enrolled \[(\d+)\]:/;
  const event = await listEventById(calendarId, eventId, eventDate, duration);
  const eventDesc = event[0].description || 'Enrolled [0]:';
  console.log('>>>> eventDesc', eventDesc);
  const eventAttendees = event[0].attendees;
  const currentDate = new Date();
  let newEventDesc = `${eventDesc} \n + ${body} \t [${currentDate.toISOString()}]`;
  let enrolledCount = newEventDesc.split('@').length - 1;
  newEventDesc = newEventDesc.replace(enrolledPattern, 'Enrolled [' + (enrolledCount) + ']:');
  eventAttendees?.push({ email: body });
  console.log('>>>>', newEventDesc);

  const googleClient = await initGoogleCalendar();
  const response = await googleClient?.events.patch({
    calendarId: calendarId,
    eventId: eventId,
    requestBody: {
      description: newEventDesc,
      //attendees: eventAttendees,    //need domain wide delegation to use this
    },    
  });
